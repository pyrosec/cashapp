...
const randomDevice = () => {
    return fingerprints_json_1.default[Math.floor(Math.random() * fingerprints_json_1.default.length)];
};
...
class CashAppClient {
    ...
    static getRandomDevice() {
        const device = fingerprints_json_1.default[Math.floor(Math.random() * fingerprints_json_1.default.length)];
    ...
    async _call(path, data, headers = {}) {
        ...
        const agent = this.proxyOptions ? new socks_proxy_agent_1.SocksProxyAgent(this.proxyOptions) : null;
    ...
    async setNotificationPreference({ appToken, sessionToken, notificationPreference, requestContext, blockerDescriptorId, }) {
        ...
        const payload = proto_1.protocol.SetNotificationPreferenceRequest.encode({
            ...
            requestContext: this.constructor.requestContextFromDescriptor(blockerDescriptorId, requestContext),
        }).finish();
    ...
    async linkCard({ card: { unencryptedPan, expiration, securityCode, postalCode, lastFour, instrumentType: instrumentTypeCard, }, paymentTokens, ocrContext, instrumentType, transferToken, requestContext, linkedViaNfc, treatment, cashFlowToken, blockerDescriptorId, cashClientScenario, }) {
        const payload = proto_1.protocol.LinkCardRequest.encode({
            ...
            instrumentType,
            transferToken,
            requestContext: this.constructor.requestContextFromDescriptor(blockerDescriptorId || "card_blocker", requestContext),
            linkedViaNfc,
            treatment,
        }).finish();
    ...
    async verifyEmail({ requestContext, emailAddress, verificationCode, cashClientScenario, cashFlowToken, blockerDescriptorId, }) {
        const payload = proto_1.protocol.VerifyEmailRequest.encode({
            requestContext: this.constructor.requestContextFromDescriptor(blockerDescriptorId || "email_verification", requestContext),
            emailAddress,
            verificationCode,
        }).finish();
    ...
    async syncEntities({ newestToken, oldestToken, allKnownRanges, isProtocolChangeSafe, trigger, }) {
        ...
        return proto_1.protocol.SyncEntitiesResponse.decode(responseToBuffer(response.data));
    }
...
CashAppClient.DEFAULT_ANDROID_VERSION = null;
